...
export function AttendanceTable({ standing, showRankInfo }: { standing: Row[]; showRankInfo: boolean }) {
  const maxPresenze = Math.max(1, ...standing.map((r) => r.presenze));
          ...
          {standing.map((row, i) => (
            ...
            >
              ...
              <td className="py-2 pr-2">
                ...
                >
                  <PlayerAvatar player={row.player} size={28} rank={showRankInfo ? i + 1 : undefined} />
                  <span className={row.player.attivo ? "" : "text-muted"}>{playerName(row.player)}</span>
                  {showRankInfo && <RankArrow delta={row.isNew ? null : row.rankDelta} />}
                  {showRankInfo && row.isNew && (
