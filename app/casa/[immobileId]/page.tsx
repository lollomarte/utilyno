...
import { getContrattoPerImmobileInquilino, getComunicazioniPerInquilino } from "@/lib/data/inquilino";
...
export default async function ImmobileDashboardPage({ params }: { params: Promise<{ immobileId: string }> }) {
  ...
  if (contesto.relazione === "PROPRIETARIO") {
    return <DashboardProprietario immobileId={immobileId} proprietarioId={contesto.proprietarioId} userId={session.user.id} />;
  }
  return <DashboardInquilino immobileId={immobileId} inquilinoId={contesto.inquilinoId} userId={session.user.id} />;
}
...
async function DashboardProprietario({
  ...
  const immobile = await getImmobileDetailForProprietario(immobileId, proprietarioId);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-0">
        ...
        <div className="p-6 pt-0">
          <Link href={`/casa/${immobileId}/pagamenti`} className="text-sm font-medium text-primary hover:underline">
            Vedi tutto
          </Link>
        </div>
      </Card>
      ...
      <Card className="p-0">
        ...
        <div className="p-6 pt-0">
          <Link href={`/casa/${immobileId}/segnalazioni`} className="text-sm font-medium text-primary hover:underline">
            Vedi tutto
          </Link>
        </div>
      </Card>
      ...
      <Card className="p-0">
        ...
        <div className="p-6 pt-0">
          <Link href={`/casa/${immobileId}/documenti`} className="text-sm font-medium text-primary hover:underline">
            Vedi tutto
          </Link>
        </div>
      </Card>
...
async function DashboardInquilino({
  ...
  const contratto = await getContrattoPerImmobileInquilino(inquilinoId, immobileId);
